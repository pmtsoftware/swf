...
continuousFetch var = do
  ...
  forkIO $ forever $ sleepThenUpdate manager var
  where
...
fetchRegistry manager = do
  ...
  case WA.metadataBlobToRegistry blobBytes now of
    Left err -> error err
    Right (This err) -> error $ "Unexpected MDS parsing errors: " <> Text.intercalate "," (NE.toList err)
